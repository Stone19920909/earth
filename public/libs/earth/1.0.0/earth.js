...
(function() {
    ...
    function init() {
        ...
        function updateAlertMarkers() {
            ...
            if (!globe || !alerts || alerts.length === 0) {
            ...
            alerts.forEach(function(alert) {
                ...
                if (!point || !_.isFinite(point[0]) || !_.isFinite(point[1])) {
                    d3.select("[data-alert-id='" + alert.id + "']").remove();
                    return;
                }
                ...
                var group = d3.select("[data-alert-id='" + alert.id + "']");
                if (group.empty()) {
                    group = foreground.append("g")
                ...
                group.select(".pulse-circle")
                    .attr("cx", point[0])
                    .attr("cy", point[1])
                    .attr("r", pulseRadius);

                group.selectAll("circle:not(.pulse-circle)")
                    .attr("cx", point[0])
                    .attr("cy", point[1])
                    .attr("r", markerRadius);
                ...
                group.select("text:nth-of-type(1)")
                    .attr("y", point[1] + 5);

                group.select("text:nth-of-type(2)")
                    .attr("y", point[1] - markerRadius - 5);
            });

            var existingIds = alerts.map(function(a) { return a.id; });
            d3.selectAll(".alert-marker").each(function() {
                var id = d3.select(this).attr("data-alert-id");
        ...
        alertsAgent.on("update", updateAlertMarkers);
        rendererAgent.on("render", updateAlertMarkers);
